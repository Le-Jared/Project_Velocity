...
def _get_client():
    ...
    if not GEMINI_ENABLED:
        raise RuntimeError("[GEMINI] Disabled by UI toggle.")
...
SUPPLIER_HINTS = {
    "meta": (
        "This is a Meta (Facebook) Ads invoice. Client codes appear in campaign names like "
        "mcspapac_MY_F0032_..._LGL_... where MY=market, LGL=client. "
        "Also check for a standalone 13–15 digit Account Id number anywhere on the page — "
        "it may appear as a loose line, not in a labelled field."
    ),
    "google": (
        "This is a Google Ads invoice. Client codes appear in campaign names like "
        "MCSP_ID_BHC_ where ID=market, BHC=client."
    ),
    "apple": (
        "This is an Apple Search Ads invoice. The client code is a 2–4 letter code near "
        "'Client :' or 'Order Number' fields."
    ),
    "adsjoy": (
        "This is an AdsJoy Digital invoice. The client code is a 2–4 letter code near "
        "'For ADSJOY DIGITAL' in the document."
    ),
}
...
def resolve_unknown_fields(
    ...
    if not GEMINI_ENABLED:
        print("  [GEMINI] Disabled by UI toggle — skipping")
        return {}
...
def enrich_extraction(
    ...
    unknown = [
        ...
        if key in resolvable and value in UNKNOWN_VALUES
    ]
